...
bool Weather::begin(TwoWire *pi2c, SettingsData &setting, int8_t oneWirePin, int8_t windDirPin, int8_t windSpeedPin,
                    int8_t rainPin) {
    bool bRet = true;
    pI2c = pi2c;
    ...
    anemometerType = anSettings.AnemometerType;
    ...
    if (oneWirePin >= 0) {
        ...
        if (sensors.getAddress(tempSensorAdr, 0)) {
            log_i("found onewire TempSensor with adr %X:%X:%X:%X:%X:%X:%X:%X ", tempSensorAdr[0], tempSensorAdr[1],
                  tempSensorAdr[2], tempSensorAdr[3], tempSensorAdr[4], tempSensorAdr[5], tempSensorAdr[6],
                  tempSensorAdr[7]);
    ...
    if (anemometerType == eAnemometer::SPARKFUN_WEATHERKIT) {
        ...
        _weather.bRain = true;
    } else if (anemometerType == eAnemometer::VOLTAGE_DIVIDER_BASED) {
        _weather.bWindDir = true;
        _weather.bWindSpeed = true;
...
float Weather::calcPressure(float p, float t, float h) {
    return h > 0 ? (float) p * pow(1 - (0.0065 * h / (t + (0.0065 * h) + 273.15)), -5.257) : p;
...
void Weather::checkAnalogAnemometer() {
    if (_weather.bWindSpeed) {
        uint16_t reading = analogRead(_windSpeedPin);
        _weather.vaneValue = reading;
        windspeed = reading * (5.0 / 1023.0) * 6.0;
        _weather.WindSpeed = windspeed;
    }
}

void Weather::checkAnalogWeatherVane() {
    if (_weather.bWindDir) {
        uint16_t vaneReading = analogRead(_windDirPin);
        _weather.vaneValue = vaneReading;
        winddir = (map(vaneReading, 0, 1023, 0, 359) + _windDirOffset) % 360;
        _weather.WindDir = winddir;
    }
}

void Weather::checkDavisAnemometer() {
    checkAnalogAnemometer();
    if (_weather.bWindSpeed) {
        if (timerIrq) {
...
void Weather::run() {
    ...
    if ((tAct - tOld) >= WEATHER_REFRESH) {
        ...
        if (anemometerType == SPARKFUN_WEATHERKIT) {
            _weather.WindSpeed = _sparkFunWeatherMeter->getWindSpeed();
            _weather.WindDir = _sparkFunWeatherMeter->getWindDirection();
        } else if (anemometerType == VOLTAGE_DIVIDER_BASED) {
            checkAnalogAnemometer();
            checkAnalogWeatherVane();
        } else {
            checkDavisAnemometer();
        }
